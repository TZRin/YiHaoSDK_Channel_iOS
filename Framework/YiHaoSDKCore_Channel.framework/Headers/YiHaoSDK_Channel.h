...
#import "YHSDKReportInfo_Channel.h"
#import "YHSDKTypedef_Channel.h"
  
@interface YiHaoSDK_Channel : NSObject

+ (YiHaoSDK_Channel *_Nullable)sharedSDK;
...
- (void)setupAggregateSDKWithGameID:(NSString *_Nonnull)gameID channelID:(NSString *_Nonnull)channelID;

/// YHYX渠道的SDK初始化,先调用上面聚合sdk初始化
...
/// 上报信息,如角色成功登陆/登出/创建等，只传递当前类型您有且能与参数模型对应的数据，其余数据字段可不传，需要在登录后调用
...
/// }YHSDKReportType;
/// @param info 数据模型
/// @param type 上报类型，参考如上说明
- (void)uploadPlayerInfo:(YHSDKReportInfo_Channel *_Nullable)info type:(YHSDKChannelReportType)type;
